...
class NNConv(tg.nn.NNConv):
	def __init__(self, in_channels, out_channels, act='relu', norm=None, bias=True, aggr='max'):
		super(NNConv, self).__init__(in_channels, out_channels, MLP([in_channels, out_channels], act, norm, bias), aggr)
		print('parameters', in_channels, out_channels)
	def forward(self, x, edge_index, edge_attr):
		print('attributes', x.shape, edge_index.shape, edge_attr.shape)
		return super(NNConv, self).forward(x, edge_index, edge_attr)
...
class GraphConv(nn.Module):
	...
	def __init__(self, in_channels, out_channels, conv='edge',
		...
		if conv.lower() == 'edge':
			...
			self.gconv = NNConv(in_channels, out_channels, act, norm, bias)
		else:
			raise NotImplementedError('conv {} is not implemented'.format(conv))

	def forward(self, x, edge_index, edge_attr = None):
		return self.gconv(x, edge_index, edge_attr)
...
class DynConv(GraphConv):
	...
	def forward(self, x, edge_attr, batch=None):
